...
class optimizer:
    ...
    def run(self, time_list_valid):
        ...
        totaal_allocatie_after_flex = pd.Series(self.model.totaal_allocatie_after_flex.extract_values(), name=self.model.totaal_allocatie_after_flex.name)
        ...
        total_forecast_weather = pd.Series(self.model.total_forecast_weather.extract_values(),name=self.model.total_forecast_weather.name)
        total_forecast_hour_weather = pd.Series(self.model.total_forecast_hour_weather.extract_values(),name=self.model.total_forecast_hour_weather.name)
        ...
        ax[5].plot(difference_MWh_plot_onvermijdbaar, label='Volume verschil onvermijdbaar',color='c')
        ax[5].plot(difference_MWh_plot_comp, label='Volume verschil comp', color='b', alpha=0.2)
        ax[5].plot(difference_MWh_plot_after_flex, label='after flex verschil', color='b', alpha=0.5)
        ...
        axes[0].plot(total_forecast, label='E programma', color='r')
        axes[0].plot(total_forecast_hour_e_programma,label='Total forecast hour E program', color='r')
        axes[0].plot(total_forecast_hour_v_programma,label='Total forecast hour V program', color='b')
        ...
        axes[0].plot(totaal_allocatie_after_flex, label='Total allocatie after flex', color='c')
        ...
        axes[1].plot(imbalance_costs_after_flex, label='Imbalance after flex', color='m', alpha=0.5)
        ...
        axes[1].plot(imbalance_costs_after_flex_epex, label='Imbalance after flex EPEX',color='g', alpha=0.5)
        ...
        axes[1].plot(imbalance_before_flex_total, label='imbalance cumulatief before', color='b')
        axes[1].plot(imbalance_costs_after_flex_total, label='imbalance cumulatief after', color='b', alpha=0.5)
