...
class SolarAPICall:
    ...
    def nc_to_csv(self, filepath, x, max_keys):
        ...
        if station_names == self.station_names_check:
            ...
            self.new_df[df['time'].loc[0]] = (df['qgn'] + df['qgx']) / 2
        else:
            ...
            for index, row in enumerate(self.new_df['stationname']):
                for index2, row2 in enumerate(df['stationname']):
                    if row == row2:
                        self.new_df.loc[index, df['time'].loc[0]] = (df.loc[index2, 'qgn'] + df.loc[index2, 'qgx']) / 2

        if x == (max_keys - 1):
            # Write to csv
            self.new_df.to_csv(f'data/solar_data/solar_data_{self.timestamp}.csv', index=True)

    def download_file_from_temporary_download_url(self, download_url, filename, x, max_keys):
        try:
            directory = "data/solar_data/solarAPIData"  # Directory name
...
run_api = SolarAPICall(timestamp=20231205)
run_api.main()